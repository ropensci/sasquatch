...
**Note**: For non-html output, use `sas_set_tempdir()` to set an temporary
directory which SAS can write intermediate files to. This is required to 
perform HTML to Markdown translation.
...
In the conversion process dates and datetimes are converted to local time. If 
utilizing another timezone, use `as.POSIXct()` or `lubridate::with_tz()` to 
convert back to the desired time zone.
